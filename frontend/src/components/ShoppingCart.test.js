...
test("Calculate total cost of items in cart", () => {
  expect(screen.getByText(`Total: $${((6.99 * 3) + 2.50).toFixed(2)}`)).toBeInTheDocument();
});
...
test("Remove item from cart", () => {
  ...
  expect(cartContextValue.removeFromCart).toHaveBeenCalledWith(cart[0].cartLines[0].cartLineId);
});
